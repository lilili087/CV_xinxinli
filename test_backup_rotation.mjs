...
import {
  createProjectBackup,
  isManagedBackupDirectoryName
} from "./scripts/create-project-backup.mjs";
...
try {
  ...
  const retainedDirectories = (await fs.readdir(backupsRoot, { withFileTypes: true }))
    ...
    .sort();
  const managedDirectories = retainedDirectories.filter((directoryName) => isManagedBackupDirectoryName(directoryName));

  assert.equal(managedDirectories.length, 5, "备份目录未限制为最近 5 份");
  assert.deepEqual(
    managedDirectories,
    createdBackups.slice(1),
    "未按时间顺序保留最近 5 份备份"
  );
  assert.ok(retainedDirectories.includes("legacy"), "非脚本生成的旧目录不应被误删");
