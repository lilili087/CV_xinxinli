...
const MANAGED_BACKUP_DIRECTORY_PATTERN = /^\d{8}-\d{6}(?:-\d+)?$/;
...
export function isManagedBackupDirectoryName(directoryName) {
  return MANAGED_BACKUP_DIRECTORY_PATTERN.test(directoryName);
}
...
export async function pruneOldBackups(backupRootDir, retentionCount = DEFAULT_RETENTION_COUNT) {
  ...
  const backupDirectories = dirents
    .filter((dirent) => dirent.isDirectory() && isManagedBackupDirectoryName(dirent.name))
